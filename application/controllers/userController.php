...
class UserController extends CI_Controller {
	...
	public function updatePassword()
	{
		...
		if ($this->form_validation->run()) {
			...
			if ($this->userModel->checkPassword($this->input->post('password'), $user['password']) ) {
				...
				$user_data = array('id' => $user['id'], 'password' => $password);
			
				if ($user = $this->userModel->update($user_data)) {
	...
	public function updateData()
	{
		...
		if ($this->form_validation->run()) {
			...
			if($user = $this->userModel->update($user_data)) {
				$this->data['update_success'] = 'Dados alterados com êxito. <a href="'.base_url('admin').'">Voltar ao seu perfil</a>';
				$this->userModel->allowPass($user);
			}
			else { $this->data['update_error'] = 'Ocorreu algum erro. Tente novamente mais tarde.'; }
		}
